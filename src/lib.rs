...
const DEFAULT_CONN_INFO_FILENAME: &str = "_node_connection_info.config";

// Relative path from $HOME where to read the connection information from
const BOOTSTRAP_CONN_INFO_PATH: &str = ".safe/node/bootstrap";
...
pub fn run_with(cmd_args: Option<&[&str]>) -> Result<(), String> {
    ...
    for i in 2..args.num_nodes + 1 {
        // Fetch contact info from $HOME/.safe/node/
        let contact_info = read_bootstrap_info(args.verbosity)?;
        ...
        let current_node_args =
            build_node_args(common_args.clone(), &node_dir, Some(&contact_info));
...
fn read_bootstrap_info(verbosity: u8) -> Result<String, String> {
    let home_dir = dirs_next::home_dir().ok_or("Home directory not found")?;
    let conn_info_path = home_dir.join(BOOTSTRAP_CONN_INFO_PATH);
    #[allow(unused_assignments)]
    let mut path_for_display = conn_info_path.clone();
    let sections = fs::read_dir(conn_info_path)
        .map_err(|err| format!("Failed to read dir {}", err))?
        .map(|res| res.map(|e| (e.path(), e.file_name())))
        .collect::<Result<Vec<_>, std::io::Error>>()
        .map_err(|err| format!("Failed to collect sections {}", err))?;

    let localhost = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1));
    let mut hard_coded_contacts = HashSet::new();
    if sections.is_empty() {
        path_for_display = home_dir
            .join(BOOTSTRAP_CONN_INFO_PATH)
            .join(DEFAULT_CONN_INFO_FILENAME);
        hard_coded_contacts.insert(SocketAddr::new(localhost, 12000));
    } else {
        let mut rng = rand::thread_rng();
        let index = rng.gen_range(0, sections.len());
        if let Some((_, section_filename)) = sections.get(index) {
            path_for_display = home_dir
                .join(BOOTSTRAP_CONN_INFO_PATH)
                .join(section_filename);
            let file = File::open(&path_for_display).map_err(|err| {
                format!(
                    "Failed to open node connection information file at '{}': {}",
                    path_for_display.display(),
                    err
                )
            })?;
            let reader = BufReader::new(file);
            hard_coded_contacts = serde_json::from_reader(reader).map_err(|err| {
                format!(
    ...
    let msg = format!("Connection info: {}", conn_info_str);
    if verbosity > 0 {
        println!("Connection info directory: {}", path_for_display.display());
        println!("{}", msg);
    }
    debug!("{}", msg);
    debug!("Connection info directory: {}", path_for_display.display());
